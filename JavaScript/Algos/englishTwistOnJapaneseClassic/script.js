...
function shiritori2(words) {
	let res = [];
	for (let w of words) {
		if (!w.length || (res.length && w[0] !== res[res.length - 1].slice(-1)))
			break;
		res.push(w);
	}
	return res;
...
function shiritori3(words) {
	if (!words.length || words[0] === '') return [];

	for (let i = 1; i < words.length; i++)
		if (words[i - 1].slice(-1) !== words[i][0]) return words.slice(0, i);
