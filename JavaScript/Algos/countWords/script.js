...
function wordCount(s) {
	const words = s.toLowerCase().match(/[a-z]+/g) || [],
		exclude = new Set(['a', 'the', 'on', 'at', 'of', 'upon', 'in', 'as']);

	return words.filter((w) => !exclude.has(w)).length;
}

function wordCount2(s) {
	regex = /[a-z]*/gi;
	const exceptions = ['a', 'the', 'on', 'at', 'of', 'upon', 'in', 'as'];

	return s.match(regex).filter((e) => !exceptions.includes(e.toLowerCase()) && e)
		.length;
}
